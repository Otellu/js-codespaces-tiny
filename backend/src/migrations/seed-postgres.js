...
async function seed() {
  ...
  const inserted = await Post.bulkCreate(docs, { returning: true });
  const first = inserted[0];

  console.log(`Seeded ${inserted.length} posts.`);
  console.log('Primary post:', { id: first.id, title: first.title, votes: first.votes });

  try {
    const envPath = path.resolve(__dirname, '../../../frontend/.env');
    const content = `REACT_APP_POST_ID=${first.id}\n`;
